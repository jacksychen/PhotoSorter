...
class TestBuildOrderedSequence:
    ...
    def test_original_indices_can_preserve_pre_filter_positions(self):
        paths = _paths(2)
        labels = np.array([0, 1])
        result = build_ordered_sequence(paths, labels, original_indices=[0, 2])
        assert [p.original_index for p in result] == [0, 2]

    def test_original_indices_length_must_match_paths(self):
        paths = _paths(2)
        labels = np.array([0, 1])
        with pytest.raises(ValueError, match="original_indices length must match paths length"):
            build_ordered_sequence(paths, labels, original_indices=[0])
