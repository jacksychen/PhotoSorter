...
class TestOutputManifest:
    ...
    def test_cluster_order_follows_ordered_sequence_not_cluster_id(self, tmp_path):
        ordered = [
            _photo(2, position=0, original_index=0, name="a.jpg"),
            _photo(2, position=1, original_index=1, name="b.jpg"),
            _photo(1, position=2, original_index=2, name="c.jpg"),
        ]
        manifest_path = tmp_path / "manifest.json"
        output_manifest(ordered, manifest_path, input_dir=Path("/input"))
        data = json.loads(manifest_path.read_text())
        assert [c["cluster_id"] for c in data["clusters"]] == [2, 1]
