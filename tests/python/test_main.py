...
def test_run_pipeline_happy_path_with_skipped_images(tmp_path, monkeypatch):
    ...
    def fake_build_ordered_sequence(paths, labels, *, original_indices=None):
        calls["build_ordered_sequence"] = (list(paths), labels.copy(), list(original_indices or []))
        return [
    ...
    filtered_paths, labels, original_indices = calls["build_ordered_sequence"]
...
def test_run_pipeline_happy_path_without_skipped_images(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda paths, labels, *, original_indices=None: [
